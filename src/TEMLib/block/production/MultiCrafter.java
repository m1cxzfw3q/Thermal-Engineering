                ...
                table.table(Styles.grayPanel, t -> {
                    ...
                    for (Recipe recipe : configRecipe) {
                        t.table(Styles.grayPanel, tl -> {
                            tl.add("[#ffd37f][" + i1[0] + "][]");
                            i1[0]++;
                            tl.add(Utils.itemsDisplay(recipe.input.items, recipe.craftTime < 0 ? uniCraftTime : recipe.craftTime));
                            tl.add(Utils.liquidsDisplay(recipe.input.liquids));
                            tl.image(Icon.right).color(Pal.darkishGray).size(40);
                            tl.add(Utils.itemsDisplay(recipe.output.items, recipe.craftTime < 0 ? uniCraftTime : recipe.craftTime));
                            tl.add(Utils.liquidsDisplay(recipe.output.liquids));
    ...
    public class MultiCrafterBuild extends Building implements HeatConsumer, HeatBlock {
        ...
        public void updateTile() {
            ...
            if (currentRecipes != null && !currentRecipes.isEmpty()) {
                ...
                if ((currentRecipe != null && !((currentRecipe.input.items.length == 0 || items.has(currentRecipe.input.items))
                            && Utils.hasLiquid(liquids, currentRecipe.input.liquids))) || !currentRecipes.contains(currentRecipe)) {
                ...
                if (currentRecipe == null) for (Recipe recipe : currentRecipes) {// 查找第一个可用的配方
                    if ((recipe.input.items.length == 0 || items.has(recipe.input.items)) && Utils.hasLiquid(liquids, recipe.input.liquids)) {
        ...
        public void displayBars(Table bars) { //动态更新热量条及流体条
            ...
            if (heatRequirement > 0) bars.add(new Bar(
                    () -> Core.bundle.format("tebar.heatrequire", (int)(heat + 0.01f), (int)(heatRequirement + 0.01f)),
                    () -> Pal.lightOrange,
